...
var fullFileString = "siteid,\tposts,\tforums_crawled,\ttotal_forums\r\n";

console.time('label');

fsp.readFileAsync("./output/final_output_sorted.txt").then(function(contents) {
    ...
    fsp.readFileAsync("./data/sorted/total_number_of_forums_sorted.csv").then(function(contents1) {
        var result1 = contents1.toString('utf8');
        var totalForumsPerSiteSplits = result1.split(/\r\n|\r|\n/);        
        var iterator = 0;        
        var siteidOutput = '';
        var siteidTotalForums = '';
        var totalForumNumber = 0;        
        ...
        for(var j=1; j < outputRequestsSplits.length; j++) {
            ...
            for(var x=1; x < totalForumsPerSiteSplits.length;  x++) {
                siteidTotalForums = String(totalForumsPerSiteSplits[x].match(/^[a-zA-Z0-9]+/i));
                if (siteidOutput === siteidTotalForums) {
                    totalForumNumber = String(totalForumsPerSiteSplits[x].match(/[0-9]+$/i));
                    (function (ors, tfn) {
                        fullFileString += `${ors},\t${tfn}\r\n`;
                    })(outputRequestsSplits[j], totalForumNumber);
                    siteidMatched = true;
                    break;
                 }
            }
        ...
        fsp.writeFileAsync("./output/final_output_total_forums_sorted.txt", finalfilestring.toString('utf-8')).then(function() {
